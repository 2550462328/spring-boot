package cn.ictt.zhanghui.springboot_test.common.queue.jvm.delayqueue;
            ...
            while (true) {
                try {
                    ...
                    if(message!=null){
                         ...
                         * 获取剩余红包个数以及金额
                         */
                        int num = (int) redisService.getValue(redPacketId+"-num");
                        int restMoney = (int) redisService.getValue(redPacketId+"-money");
                        LOGGER.info("剩余红包个数{}，剩余红包金额{}",num,restMoney);
                        /**
                         * 清空红包数据
                         */
                        redisService.removeValue(redPacketId+"-num");
                        redisService.removeValue(redPacketId+"-money");
                        /**
                         * 异步更新数据库、异步退回红包金额
                         */
                    }
